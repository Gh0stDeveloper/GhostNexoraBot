...
const alertCenter = read('apps/web/components/ops-alert-center.tsx')
const opsData = read('apps/web/lib/ops.ts')
...
assert.match(operations, /runtime=\{snapshot\.runtime\}/, 'Operational alert center must receive the full runtime state')
assert.match(opsData, /reportedConnected: boolean/, 'Runtime snapshot must preserve the last reported connected state separately from freshness')
assert.match(opsData, /connected: reportedConnected && fresh/, 'Effective runtime connectivity must still require a fresh heartbeat')
assert.match(alertCenter, /runtime\.reportedConnected && runtime\.updatedAt > 0 && !runtime\.fresh/, 'Heartbeat alert must only fire for a runtime that was actually reported connected')
assert.match(alertCenter, /runtime\.updatedAt \+ 180_000/, 'Heartbeat alert timestamp must derive from the missed heartbeat deadline')
