...
const providersDashboard = read('apps/web/components/providers-dashboard.tsx')
...
assert.match(subbotPage, /sectionIds: SubbotSection\[\] = \['overview', 'platforms', 'providers', 'commands', 'groups', 'logs', 'jobs', 'audit', 'diagnostics', 'account'\]/, 'Subbot diagnostics route must remain explicitly allowed alongside isolated E10 logs and E11 jobs')
...
assert.equal(/provider\.averageLatencyMs/.test(diagnostics), false, 'Detailed provider telemetry must move out of Developer Diagnostics after E4')
assert.match(providersDashboard, /averageLatencyMs/, 'Dedicated Providers dashboard must own detailed provider telemetry')
