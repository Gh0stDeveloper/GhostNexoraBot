...
CURRENT_UPDATE_STAGE="fetch"
CURRENT_UPDATE_PROGRESS=2
LAST_ERROR_LOG=""
NEW_SHA=""
NEW_VERSION=""
...
update_progress() {
  local stage="$1" status="$2" progress="$3" message="${4:-}" error="${5:-}"
  [[ -f "${PROGRESS_WRITER}" ]] || return 0
  env INSTALL_DIR="${INSTALL_DIR}" STATE_DIR="${STATE_DIR}" DATA_DIR="${DATA_DIR}" UPDATE_JOB_ID="${UPDATE_JOB_ID}" \
    node "${PROGRESS_WRITER}" \
      ...
      --old-sha "${OLD_SHA:-}" \
      --new-sha "${NEW_SHA:-}" \
      --version "${NEW_VERSION:-}" >/dev/null 2>&1 || true
}
...
section '1/8 · Código'
CURRENT_UPDATE_STAGE="fetch"
CURRENT_UPDATE_PROGRESS=5
LAST_ERROR_LOG=""
update_progress fetch running 5 'Consultando el repositorio remoto.'
git fetch origin "${BRANCH}"
git checkout "${BRANCH}"
git pull --ff-only origin "${BRANCH}"
NEW_SHA="$(git rev-parse HEAD)"
NEW_VERSION="$(node -p "require('./package.json').version" 2>/dev/null || echo desconocida)"
CURRENT_UPDATE_PROGRESS=18
update_progress fetch running 18 "Código actualizado a ${NEW_SHA:0:12}."
...
CURRENT_UPDATE_STAGE="dependencies"
CURRENT_UPDATE_PROGRESS=30
LAST_ERROR_LOG="/tmp/ghost-nexora-npm-install.log"
update_progress dependencies running 30 'Sincronizando dependencias Node.'
...
CURRENT_UPDATE_PROGRESS=43
update_progress dependencies running 43 'Dependencias sincronizadas.'

section '5/8 · Build'
CURRENT_UPDATE_STAGE="build"
CURRENT_UPDATE_PROGRESS=50
LAST_ERROR_LOG="/tmp/ghost-nexora-build.log"
update_progress build running 50 'Compilando Bot y componentes habilitados.'
if [[ "${WEB_ENABLED}" -eq 1 ]]; then
...
CURRENT_UPDATE_PROGRESS=64
update_progress build running 64 'Build completado.'

CURRENT_UPDATE_STAGE="migration"
CURRENT_UPDATE_PROGRESS=70
LAST_ERROR_LOG=""
update_progress migration running 70 'Preparando permisos, servicios y migraciones de runtime.'
...
CURRENT_UPDATE_PROGRESS=78
update_progress migration running 78 'Preparación de migraciones y servicios completada.'

section '8/8 · Reinicio selectivo de servicios'
CURRENT_UPDATE_STAGE="restart"
CURRENT_UPDATE_PROGRESS=82
update_progress restart running 82 'Reiniciando servicios con el nuevo build.'
...
CURRENT_UPDATE_PROGRESS=90
update_progress restart running 90 'Servicios reiniciados; iniciando healthcheck.'
CURRENT_UPDATE_STAGE="healthcheck"
CURRENT_UPDATE_PROGRESS=93
update_progress healthcheck running 93 'Verificando servicios y endpoint local.'
sleep 3
...
BOT_HEALTH_PORT_VALUE="${BOT_HEALTH_PORT_VALUE:-$(grep '^BOT_HEALTH_PORT=' .env 2>/dev/null | tail -n1 | cut -d= -f2- || echo 3001)}"
HEALTH_HTTP="000"
for _attempt in $(seq 1 15); do
  HEALTH_HTTP="$(curl -sS -o /tmp/ghost-nexora-health.json -w '%{http_code}' "http://127.0.0.1:${BOT_HEALTH_PORT_VALUE}/health" 2>/dev/null || echo 000)"
  if [[ "${HEALTH_HTTP}" == "200" || "${HEALTH_HTTP}" == "503" ]]; then break; fi
  sleep 1
done
if [[ "${HEALTH_HTTP}" != "200" && "${HEALTH_HTTP}" != "503" ]]; then
  LAST_ERROR_LOG="/tmp/ghost-nexora-health.json"
  fail "El endpoint /health no respondió después del reinicio (HTTP ${HEALTH_HTTP})."
  false
fi

CURRENT_UPDATE_PROGRESS=100
update_progress healthcheck completed 100 "Actualización completada. Health HTTP ${HEALTH_HTTP}; Bot=${BOT_STATE}; Web=${WEB_STATE}; LLM=${LLM_STATE}."
