...
try {
  ...
  economy.db.prepare('UPDATE raids_v4 SET hp = 1 WHERE id = ?').run(raid.id)
  const attack = world.raidAttack(group, alice)
  assert.equal(attack.ok, true)
  assert.equal(attack.defeated, true)
  const raidRows = world.raidMembers(raid.id)
  assert.equal(raidRows.length, 2)
  assert.equal(raidRows.every((row) => row.rewarded === 1), true)
