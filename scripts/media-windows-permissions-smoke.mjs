...
const configSource = read('apps/bot/src/config.ts')
const settingsSource = read('apps/bot/src/core/settings.ts')
const humanBehavior = read('apps/bot/src/services/human-behavior-v8.ts')
const automaticResponsesSource = read('apps/bot/src/commands/automatic-responses.ts')
const whatsappRateLimit = read('apps/bot/src/services/whatsapp-rate-limit.ts')
const whatsappAdapter = read('apps/bot/src/platform/whatsapp/adapter.ts')
const typingSource = read('apps/bot/src/core/typing.ts')
const aiSource = read('apps/bot/src/commands/ai.ts')
const lempiSource = read('apps/bot/src/commands/lempi-api.ts')
...
assert.match(configSource, /AUTO_REACT: z\.string\(\)\.default\('false'\)/, 'Human reactions must default to disabled')
assert.match(settingsSource, /humanReactionsEnabled: false/, 'Persistent human reactions setting must default to false')
assert.match(settingsSource, /setHumanReactionsEnabled/, 'Human reactions runtime toggle missing')
assert.match(humanBehavior, /settings\.humanReactionsEnabled/, 'Human behavior must honor the runtime reactions toggle')
assert.match(automaticResponsesSource, /autorespuestas reacciones on\|off\|status/, 'Human reactions control must stay inside the existing autorespuestas command')
assert.match(whatsappRateLimit, /isWhatsAppRateOverlimit/, 'Central WhatsApp rate-overlimit detector missing')
assert.match(whatsappRateLimit, /auxiliary activity paused/, 'WhatsApp auxiliary backoff logging missing')
assert.match(whatsappAdapter, /tryWhatsAppAuxiliaryAction/, 'Command reactions must use rate-limit backoff')
assert.match(whatsappAdapter, /typing: false/, 'WhatsApp typing capability must be disabled')
assert.doesNotMatch(typingSource, /setInterval|setTyping\(/, 'Typing indicator must not emit periodic presence')
assert.doesNotMatch(aiSource, /sendPresenceUpdate\('composing'/, 'AI command must not emit direct composing presence')
assert.doesNotMatch(lempiSource, /sendPresenceUpdate\('composing'/, 'Lempi commands must not emit direct composing presence')
