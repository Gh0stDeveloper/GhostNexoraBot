...
function Require-Install {
  if (-not (Test-Path (Join-Path $InstallDir '.git'))) {
    throw "No se encontró Ghost Nexora Bot en $InstallDir. Ejecuta primero install-windows.ps1."
  }
  if (-not (Test-Path $EnvFile)) {
    throw "No existe $EnvFile. Repite el instalador para reparar la configuración."
  }
}
...
function Require-Runtime {
  ...
  if (-not (Test-Path $BotEntry)) {
    throw 'El build no existe. Ejecuta: ghostnexora update'
  }
}
...
function Start-Bot {
  ...
  if (Test-ProcessId $pidValue) {
    Write-Info "MainBot ya está activo (PID $pidValue)."
    return
  }
...
function Start-Web {
  ...
  if (-not (Test-Path $nextBuild)) { throw 'El build web no existe. Ejecuta: ghostnexora update' }
  $pidValue = Get-StoredPid $WebPidFile
  if (Test-ProcessId $pidValue) {
    Write-Info "Dashboard ya está activo (PID $pidValue)."
    return
  }
  ...
  try {
    ...
    $process = Start-Process -FilePath 'npm.cmd' `
      -ArgumentList @('run', 'start', '--workspace=@ghostnexora/web') `
      -WorkingDirectory $InstallDir `
      -WindowStyle Hidden `
...
function Show-Status {
  ...
  Write-Host ('Dashboard  : ' + $(if (Test-ProcessId $webPid) { "ONLINE · PID $webPid" } else { 'OFFLINE' }))
  ...
  try {
    ...
    if ($null -ne $health.llm) {
      Write-Host ('LLM local   : ' + $(if ($health.llm.localEnabled) { 'ON · ' + $health.llm.model } else { 'OFF' }))
    }
    if ($null -ne $health.subbots) {
      Write-Host ("Subbots     : $($health.subbots.online)/$($health.subbots.total) online")
    }
  } catch {
    Write-Warn 'Health endpoint no disponible todavía.'
  }
}
...
function Update-Bot {
  ...
  Write-Info 'Sincronizando dependencias…'
  ...
  if ($webWasRunning) { Start-Web }
  Write-Ok 'Actualización completada. Sesiones y datos se conservaron.'
}
...
function Doctor {
  ...
  foreach ($check in $checks) {
    ...
    if ($null -eq $cmd) {
      Write-Warn ($check.Name + ': no disponible')
      continue
    }
    try {
      ...
      Write-Warn ($check.Name + ': instalado, pero no respondió correctamente')
    }
  }
...
function Show-Help {
  ...
  Write-Host '  status            Estado, health, LLM y subbots'
  Write-Host '  logs              Sigue los logs del MainBot'
  Write-Host '  pair <numero>     Vincula WhatsApp por pairing code'
  Write-Host '  web-start         Inicia el dashboard local'
  Write-Host '  web-stop          Detiene el dashboard local'
  Write-Host '  update            Actualiza main y recompila sin borrar datos'
