...
const mfaChallenge = read('apps/web/components/mfa-challenge.tsx')
const totpVerify = read('apps/web/app/api/auth/totp/verify/route.ts')
...
assert.match(security, /CREATE TABLE IF NOT EXISTS web_totp/, 'Encrypted TOTP store missing')
assert.match(security, /aes-256-gcm/, 'TOTP secrets must be encrypted with AES-GCM')
assert.match(security, /createHmac\('sha1'/, 'RFC-compatible HOTP/TOTP HMAC-SHA1 generator missing')
assert.match(security, /principalHasVerifiedTotp/, 'TOTP second-factor availability helper missing')
...
assert.match(securityCenter, /totp_begin/, 'TOTP enrollment UI missing')
assert.match(securityCenter, /totp_confirm/, 'TOTP confirmation UI missing')
assert.match(mfaChallenge, /api\/auth\/totp\/verify/, 'MFA chooser must support TOTP verification')
assert.match(totpVerify, /verifyTotpForPrincipal/, 'TOTP MFA verification endpoint missing')
