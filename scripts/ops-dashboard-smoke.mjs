...
try {
  ...
  assert.equal(backupArchive.schemaVersion, 2, 'default operational backup must use the E13 archive schema')
  assert.equal(backupArchive.backupType, 'full', 'default operational backup must remain a full snapshot')
  assert.equal(backupArchive.source?.instance, 'main')
  assert.ok(backupArchive.files?.some((file) => file.name === 'ghostnexora.sqlite'), 'full backup must include the main operational database')
  assert.ok(backupArchive.files?.some((file) => file.name === 'nexora-economy.sqlite'), 'full backup must include the global economy database')
  const archivedSessions = backupArchive.files?.filter((file) => String(file.name).startsWith('sessions/')) ?? []
  assert.equal(Boolean(backupArchive.source?.sessionIncluded), archivedSessions.length > 0, 'E13 sessionIncluded must match the actual archive manifest')
  ...
  const operationsOverviewSource = await readFile(new URL('../apps/web/components/operations-overview.tsx', import.meta.url), 'utf8')
  const developerDiagnosticsSource = await readFile(new URL('../apps/web/components/developer-diagnostics.tsx', import.meta.url), 'utf8')
  const providersDashboardSource = await readFile(new URL('../apps/web/components/providers-dashboard.tsx', import.meta.url), 'utf8')
  ...
  assert.ok(backupServiceSource.includes("type BackupType = 'economy' | 'configuration' | 'subbots' | 'sessions' | 'groups' | 'full'"), 'E13 backup service must expose typed backup scopes')
  assert.ok(backupServiceSource.includes('sessionIncluded:'), 'backup manifest must explicitly declare whether sessions are included')
  ...
  assert.ok(operationsOverviewSource.includes('providerIssues'), 'operational overview must surface provider incidents')
  assert.equal(developerDiagnosticsSource.includes("x('diagnostics.providerTelemetry')"), false, 'provider product telemetry must no longer be duplicated in developer diagnostics')
  assert.ok(providersDashboardSource.includes("t('provider.dashboardTitle')"), 'dedicated providers dashboard must render localized provider telemetry')
  assert.ok(providersDashboardSource.includes('circuitState'), 'dedicated providers dashboard must expose circuit-breaker state')
