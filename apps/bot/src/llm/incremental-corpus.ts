...
function parseNewRecords(buffer: Buffer): VectorRecord[] {
  ...
  while (offset + 6 <= buffer.length) {
    ...
    if (offset + DIM * 4 + textLength > buffer.length) break
...
export function migrateLegacyVectors() {
  if (!fs.existsSync(VECTORS_FILE)) return 0
...
function nextVectorId() {
  const count = migrateLegacyVectors()
  if (!fs.existsSync(VECTORS_FILE)) return 1
  const buffer = fs.readFileSync(VECTORS_FILE)
  if (!buffer.subarray(0, MAGIC.length).equals(MAGIC)) return count + 1
  const records = parseNewRecords(buffer)
...
export function countVectors() { return migrateLegacyVectors() }
...
async function extractFile(filePath: string): Promise<string> {
  ...
  if (ext === '.pdf') { const mod = await import('pdf-parse') as unknown as { default?: new (options: { data: Uint8Array }) => { getText(): Promise<{ text: string }> } }; if (!mod.default) throw new Error('pdf-parse no expone PDFParse.'); const parser = new mod.default({ data: fs.readFileSync(filePath) }); return clean((await parser.getText()).text) }
...
export function countCorpusDocuments() { if (!fs.existsSync(CORPUS_DIR)) return 0; let count = 0; const walk=(dir:string)=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,entry.name);if(entry.isDirectory())walk(full);else if(SUPPORTED.has(path.extname(entry.name).toLowerCase()))count++}}; walk(CORPUS_DIR); return count }
