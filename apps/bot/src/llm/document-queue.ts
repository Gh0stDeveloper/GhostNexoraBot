...
function ensureDirs() { fs.mkdirSync(INBOX, { recursive: true }) }
...
function writeState(state: QueueState) {
  ensureDirs(); const temp = `${QUEUE_FILE}.tmp`; fs.writeFileSync(temp, JSON.stringify(state, null, 2)); fs.renameSync(temp, QUEUE_FILE)
}
function safeFilename(name: string) { return name.replace(/[^\p{L}\p{N}._-]/gu, '_').trim().slice(0, 120) || 'document.txt' }
...
export async function enqueueDocumentFromWhatsApp(message: WAMessage) {
  ...
  fs.writeFileSync(temp, media.buffer); fs.renameSync(temp, target)
  const state = readState()
  const job: LlmDocumentJob = { id, filename, path: target, mimetype: media.mimetype ?? null, status: 'queued', createdAt: new Date().toISOString() }
  state.jobs.push(job); writeState(state)
...
export function getQueueState() { return readState() }
export function getQueueStats() {
  const jobs = readState().jobs
  return { total: jobs.length, queued: jobs.filter((j) => j.status === 'queued').length, processing: jobs.filter((j) => j.status === 'processing').length, completed: jobs.filter((j) => j.status === 'completed').length, failed: jobs.filter((j) => j.status === 'failed').length }
}
export function updateDocumentJob(id: string, patch: Partial<LlmDocumentJob>) {
  const state = readState(); const job = state.jobs.find((item) => item.id === id); if (!job) return false
  Object.assign(job, patch); writeState(state); return true
}
