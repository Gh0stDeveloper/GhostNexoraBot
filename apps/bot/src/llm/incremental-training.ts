...
export function loadVocab(): string[] {
  ...
  const parsed = JSON.parse(fs.readFileSync(VOCAB_FILE, 'utf8')) as { vocab?: unknown }
...
/** Preserve every existing token and append only new tokens. Never rebuilds/reorders the old vocabulary. */
export function expandVocab(texts: string[]): { oldSize: number; newSize: number; added: number } {
  ...
  const additions = [...counts.entries()]
    ...
    .slice(0, Math.max(0, MAX_VOCAB - oldVocab.length))
...
function parseModel(buffer: Buffer) {
  ...
  return { newline, meta, payloadOffset, oldVocabSize, oldEmbeddingBytes, matrixBytes, oldOutputBytes, biasBytes }
}

/** Expand model.bin without reinitializing existing weights. New rows/columns are seeded deterministically. */
...
export function checkpointRound(round: number) {
  ...
  const modelDest = path.join(CHECKPOINT_DIR, `model-${round}.bin`)
  const vocabDest = path.join(CHECKPOINT_DIR, `vocab-${round}.json`)
...
export function restoreCheckpoint(round: number) {
  ...
  const modelSource = path.join(CHECKPOINT_DIR, `model-${round}.bin`)
  const vocabSource = path.join(CHECKPOINT_DIR, `vocab-${round}.json`)
...
export function checkpointPaths() {
  ...
  return fs.readdirSync(CHECKPOINT_DIR).sort()
}
