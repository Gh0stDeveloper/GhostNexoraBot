...
const SUPPORTED_RE = /\.(txt|md|csv|tsv|json|xml|html|htm|pdf|docx)$/i
...
export async function prepareCorpusAndTrain() {
  ...
  const embedder = new Embedding(); embedder.train(sentences)
  ...
  const totalEstimate = sentences.reduce((total, sentence) => total + Math.max(0, encode(sentence, vocab!, index).length - 1), 0) * EPOCHS
  let steps = 0; let lossTotal = 0; let lossCount = 0
...
function countSupportedFiles(dir: string): number {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (SUPPORTED_RE.test(entry.name)) total++
  }
  return total
}

if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(new URL(import.meta.url).pathname)) {
  prepareCorpusAndTrain().then((result) => console.log(JSON.stringify(result, null, 2))).catch((error) => { console.error(error); process.exitCode = 1 })
}
