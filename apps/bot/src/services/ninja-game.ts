/** HTML interactivo Fruit Slice / Ninja (canvas) para rich message de WhatsApp. */
export function buildNinjaGameHtml(): string {
...
<div id="status" style="position:absolute;bottom:6px;left:0;right:0;text-align:center;font-size:9px;color:rgba(255,255,255,.75);pointer-events:none;text-shadow:0 1px 4px rgba(0,0,0,.9)">3 ❤️ · Desliza para cortar, evita las bombas</div>
</div></div>
<script>
...
statusEl.textContent='3 ❤️ · Desliza para cortar, evita las bombas'
}
...
x.font='12px Arial';x.fillText('Puntos '+Math.floor(score)+' · Toca para reiniciar',W/2,H/2+16);
x.textAlign='left'
}
}
function update(){
...
bestEl.textContent='BEST '+best;
statusEl.textContent=gameOver?'Perdiste · Toca para reiniciar':lives+' ❤️ · Combo x'+Math.max(1,combo);
}
