/** HTML interactivo Snake (canvas) para rich message de WhatsApp. */
...
<div id="status" style="position:absolute;bottom:6px;left:0;right:0;text-align:center;font-size:9px;color:rgba(255,255,255,.75);pointer-events:none;text-shadow:0 1px 4px rgba(0,0,0,.9)">Flechas para empezar</div>
...
statusEl.textContent='Flechas para empezar'
}
function spawnFood(){
let f;
...
x.font='11px Arial';x.fillText('Puntos '+score+' · Toca para jugar de nuevo',c.width/2,c.height/2+14);
x.textAlign='left'
}
}
function loop(t){
