/** HTML interactivo del Dino Runner (canvas) para rich message de WhatsApp. */
export function buildDinoRunnerHtml(): string {
  return `<style>*{-webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none}</style>
...
<div><div style="font-size:11px;letter-spacing:1.5px;color:rgba(255,255,255,.45)">GHOST NEXORA</div><div style="font-size:21px;font-weight:bold;color:#fff">Dino Runner</div></div>
...
<div id="status" style="text-align:center;margin-top:10px;font-size:12px;color:rgba(255,255,255,.55)">Speed 5.0x · Toca para saltar</div>
</div></div></div>
<script>
...
statusEl.textContent='Speed 5.0x · Toca para saltar'
}
...
x.font='14px Arial';x.fillText('Toca para jugar de nuevo',c.width/2,112);
x.textAlign='left'
}
}
function loop(t){
...
bestEl.textContent='BEST '+String(Math.floor(best)).padStart(5,'0');
statusEl.textContent='Speed '+speed.toFixed(1)+'x · Toca para saltar';
