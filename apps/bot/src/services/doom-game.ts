export function buildDoomGameHtml(): string {
...
<div id="status" style="position:absolute;bottom:6px;left:0;right:0;text-align:center;font-size:9px;color:rgba(255,255,255,.75);pointer-events:none;text-shadow:0 1px 4px rgba(0,0,0,.9)">5 enemigos restantes</div>
...
x.fillStyle='#fff';x.font='12px Arial';x.fillText('Puntos '+player.score+' · Toca para reiniciar',W/2,H/2+14);
...
statusEl.textContent=win?'¡Nivel completado!':gameOver?'Has muerto':enemies.filter(e=>!e.dead).length+' enemigos restantes'
}
