...
function hashVector(text: string) {
  ...
  for (let i = 0; i < lower.length; i++) {
    ...
    const slot = (a * 31 + b * 17 + i * 13) % DIM; v[slot] += ((a % 97) + 1) / 100
  }
...
function readBinary() {
  ...
  while (offset + 6 <= buf.length) {
    const id = buf.readUInt32LE(offset); const textLen = buf.readUInt16LE(offset + 4); offset += 6; const bytes = DIM * 4
    if (offset + bytes + textLen > buf.length) break
    const vector = new Float32Array(DIM); for (let i = 0; i < DIM; i++) vector[i] = buf.readFloatLE(offset + i * 4)
    offset += bytes; const text = buf.subarray(offset, offset + textLen).toString('utf8'); offset += textLen; out.push({ id, vector, text })
  }
  return out
}
...
function trainVocab(texts: string[]) {
  const counts = new Map<string, number>()
  for (const text of texts) for (const token of tokenize(text)) counts.set(token, (counts.get(token) ?? 0) + 1)
  const vocab = ['<unk>', '<bos>', '<eos>', ...[...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, VOCAB_LIMIT - 3).map(([token]) => token)]
  fs.writeFileSync(VOCAB_FILE, JSON.stringify({ version: 2, vocab, generatedAt: new Date().toISOString() }, null, 2)); return vocab
}
function readVocab() {
  try { return (JSON.parse(fs.readFileSync(VOCAB_FILE, 'utf8')) as { vocab?: string[] }).vocab ?? [] }
  catch { return [] }
}
function positional(position: number, index: number) {
  const angle = position / Math.pow(10000, (2 * Math.floor(index / 2)) / DIM)
...
function initModel(vocabSize: number): Model {
  const size = Math.max(4, vocabSize); const make = (length: number, scale: number, salt: number) => { const a = new Float32Array(length); for (let i = 0; i < length; i++) a[i] = (seeded(i + salt) - 0.5) * scale; return a }
...
function saveModel(model: Model) {
  ...
  const payload = [model.embeddings, model.wq, model.wk, model.wv, model.wo, model.output, model.bias].map((a) => Buffer.from(a.buffer, a.byteOffset, a.byteLength))
...
function loadModel(vocabSize: number): Model {
  try {
    ...
    const meta = JSON.parse(buf.subarray(0, split).toString('utf8')) as { version?: number; vocabSize?: number; dim?: number; heads?: number }
    if (meta.version !== 2 || meta.vocabSize !== vocabSize || meta.dim !== DIM || meta.heads !== HEADS) return initModel(vocabSize)
    let offset = split + 1
    const read = (len: number) => { const a = new Float32Array(len); for (let i = 0; i < len; i++) a[i] = buf.readFloatLE(offset + i * 4); offset += len * 4; return a }
    return { vocabSize, dim: DIM, heads: HEADS, embeddings: read(vocabSize * DIM), wq: read(DIM * DIM), wk: read(DIM * DIM), wv: read(DIM * DIM), wo: read(DIM * DIM), output: read(vocabSize * DIM), bias: read(vocabSize) }
  } catch { return initModel(vocabSize) }
}
function matVec(matrix: Float32Array, input: Float32Array, rows: number, cols: number) {
  const out = new Float32Array(rows)
  for (let r = 0; r < rows; r++) { let sum = 0; const base = r * cols; for (let c = 0; c < cols; c++) sum += matrix[base + c]! * input[c]!; out[r] = sum }
  return out
}
function relu(x: number) { return x > 0 ? x : 0 }

function forward(model: Model, ids: number[]) {
  const tokens = ids.slice(-MAX_CONTEXT); const states: Float32Array[] = []
  for (let p = 0; p < tokens.length; p++) {
    const h = new Float32Array(DIM); const base = tokens[p]! * DIM
...
function trainStep(model: Model, ids: number[], learningRate = 0.001, momentum = 0.9) {
  ...
  for (let i = 0; i < model.vocabSize; i++) {
    const grad = pass.probs[i]! - (i === target ? 1 : 0); const base = i * DIM
    for (let j = 0; j < DIM; j++) model.output[base + j] -= learningRate * grad * pass.hidden[j]!
...
async function addDocument(socket: WASocket, message: WAMessage) {
  ...
  const name = media.fileName || 'document.txt'; const ext = path.extname(name).toLowerCase(); let text = ''
  ...
  ensureDirs(); const safe = name.replace(/[^\p{L}\p{N}._-]/gu, '_').slice(0, 100); fs.writeFileSync(path.join(CORPUS_DIR, `${Date.now()}-${safe}.txt`), parts.join('\n\n'))
  const records = readBinary(); let nextId = records.length ? Math.max(...records.map((x) => x.id)) + 1 : 1
  appendBinary(parts.map((part) => ({ id: nextId++, vector: hashVector(part), text: part }))); trainVocab([...parts, ...records.slice(-1000).map((x) => x.text)])
  const s = getState(); s.totalDocuments += 1; s.totalChunks += parts.length; saveState(s)
...
function addLive(text: string) {
  ...
  appendBinary(parts.map((part) => ({ id: nextId++, vector: hashVector(part), text: part }))); if (parts.length) trainVocab([...parts, ...records.slice(-250).map((x) => x.text)])
  const s = getState(); s.totalMessages += 1; s.totalChunks += parts.length; saveState(s)
}
...
async function train(reason = 'manual') {
  ...
  const vocab = readVocab(); const records = readBinary().slice(-MAX_TRAIN_RECORDS); const sequences = records.map((x) => tokenize(x.text).map((token) => vocab.indexOf(token)).filter((id) => id >= 0)).filter((ids) => ids.length >= 2)
  if (!vocab.length || !sequences.length) return { started: false, reason: 'no_training_data' as const }
  const epochs = 2; const totalSteps = sequences.reduce((sum, ids) => sum + Math.max(1, ids.length - 1), 0) * epochs
  updateProgress({ learning: true, currentProgress: 0, currentStep: 0, currentTotalSteps: totalSteps, currentEpoch: 0, currentTotalEpochs: epochs, currentMessage: `Preparando ${sequences.length} secuencias` })
  const model = loadModel(vocab.length); let lossTotal = 0; let steps = 0; const startedAt = Date.now()
  try {
    for (let epoch = 1; epoch <= epochs; epoch++) {
      updateProgress({ currentEpoch: epoch, currentMessage: `Entrenando época ${epoch}/${epochs}` })
    ...
    saveModel(model)
    const s = getState(); s.trainRuns++; s.trainSteps += steps; s.trainedMessages = s.totalMessages; s.modelVersion++; s.lastTrainAt = new Date().toISOString(); s.lastTrainDurationMs = Date.now() - startedAt; s.lastLoss = steps ? lossTotal / steps : null; s.bestLoss = s.lastLoss !== null && (s.bestLoss === null || s.lastLoss < s.bestLoss) ? s.lastLoss : s.bestLoss; s.learning = false; s.currentProgress = 100; s.currentMessage = `Completado: ${steps} pasos`; saveState(s)
    return { started: true, reason, steps, loss: s.lastLoss, durationMs: s.lastTrainDurationMs }
  } catch (error) {
    updateProgress({ learning: false, currentMessage: 'Entrenamiento detenido por error' }); throw error
  }
}
function stats() {
  const s = getState(); let storageBytes = 0
  for (const file of [STATE_FILE, VOCAB_FILE, VECTORS_FILE, MODEL_FILE, LIVE_FILE]) { try { storageBytes += fs.statSync(file).size } catch {} }
  return { ...s, pendingMessages: Math.max(0, s.totalMessages - s.trainedMessages), vectorRecords: readBinary().length, vocabSize: readVocab().length, storageBytes }
}
