...
function page(value?: string) {
  ...
  return Number.isInteger(n) && n > 0 ? Math.min(50, n) : 1
}
...
function looksUsableThumbnail(value?: string) {
  if (!value) return false
  try {
    const url = new URL(value)
    const name = `${url.pathname}${url.search}`.toLowerCase()
    return /^https?:$/.test(url.protocol) && !/placeholder|default|spacer|blank|transparent|avatar|logo\./.test(name)
  } catch {
    return false
  }
}
...
async function enrich(item: HentaiItem): Promise<HentaiItem> {
  try {
    ...
    if (PROHIBITED.test(html.slice(0, 250_000))) return item
    ...
    const image =
      ...
      $('video source').attr('data-poster') ||
      item.thumbnail
    if (!looksUsableThumbnail(image)) return item
    return { ...item, thumbnail: new URL(image!, response.url).toString() }
  } catch {
    return item
  }
}
...
async function show(ctx: CommandContext, mode: 'hot' | 'new' | 'search', currentPage: number, query?: string) {
  ...
  const cards = items.map((item, i) => ({
    title: `#${i + 1} · ${item.title}`.slice(0, 80),
    body: [
    ...
    buttons: [
      { type: 'reply' as const, text: 'Descargar', id: `${ctx.prefix}hentai dl ${item.token}` },
      { type: 'url' as const, text: 'Abrir', url: item.url },
      ...(i === items.length - 1 && items.length === PAGE_SIZE
        ? [{ type: 'reply' as const, text: 'Siguiente', id: mode === 'search'
          ? `${ctx.prefix}hentai search64 ${currentPage + 1} ${pack(query ?? '')}`
          : `${ctx.prefix}hentai ${mode} ${currentPage + 1}` }]
        : []),
    ] satisfies InteractiveButton[],
  }))

  await sendCarousel(ctx.socket, ctx.chatId, ctx.message, {
    title: `HENTAI · ${mode === 'search' ? 'BÚSQUEDA' : mode.toUpperCase()} · ${currentPage}`,
    body: mode === 'search' ? `Resultados para: ${query}` : `Explorar ${mode === 'hot' ? 'HOT' : 'NEW'}`,
    footer: '8 resultados por página',
    cards,
  })
}
...
async function editStatus(ctx: CommandContext, status: unknown, text: string) {
  ...
  if (!key) return
  try {
    await ctx.socket.sendMessage(ctx.chatId, { text }, { edit: key as never })
  } catch {
    /* best effort */
  }
}
...
async function download(ctx: CommandContext, value: string) {
  ...
  try {
    ...
    const caption = [
      `🔞 *HENTAI*`,
      result.title,
      `Peso: ${(result.size / 1024 / 1024).toFixed(1)} MB`,
    ].join('\n')
    try {
      await ctx.socket.sendMessage(
        ctx.chatId,
        { video: { url: result.filePath }, mimetype: 'video/mp4', caption },
        { quoted: ctx.message },
      )
    } catch {
      await ctx.socket.sendMessage(
        ctx.chatId,
        { document: { url: result.filePath }, mimetype: 'video/mp4', fileName: result.fileName, caption },
        { quoted: ctx.message },
      )
    }
...
export const hentaiV9Commands: BotCommand[] = [{
  ...
  aliases: ['h', 'hanime'],
  category: 'adult',
  description: 'Busca y descarga contenido hentai público con carrusel y navegación paginada.',
  ...
  async handler(ctx) {
    ...
    if (action === 'search') {
      ...
      await show(ctx, 'search', 1, query)
      return
    }
    if (action === 'search64') {
      await show(ctx, 'search', page(ctx.args[1]), unpack(ctx.args[2] ?? ''))
      return
    }
    if (action === 'dl' || action === 'download') {
      const value = ctx.args[1]
      if (!value) throw new Error(`Uso: ${ctx.prefix}hentai dl <token|url>`)
      await download(ctx, value)
      return
    }
    const raw = ctx.argText.trim()
    if (/^https?:\/\//i.test(raw)) { await download(ctx, raw); return }
