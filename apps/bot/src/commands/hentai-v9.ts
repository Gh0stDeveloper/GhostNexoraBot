...
const UA = 'Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36 Chrome/131 Safari/537.36 GhostNexoraBot/1.4'
...
async function show(ctx: CommandContext, mode: 'hot' | 'new' | 'search', currentPage: number, query?: string) {
  ...
  const cards = items.map((item, i) => {
    const buttons: InteractiveButton[] = [
      { type: 'reply', text: '⬇️ Descargar', id: `${ctx.prefix}hentai dl ${item.token}` },
      { type: 'url', text: '🌐 Abrir', url: item.url },
    ]
    if (i === items.length - 1 && items.length === PAGE_SIZE) buttons.push({
      type: 'reply', text: 'Siguiente ▶️',
      id: mode === 'search' ? `${ctx.prefix}hentai search64 ${currentPage + 1} ${pack(query ?? '')}` : `${ctx.prefix}hentai ${mode} ${currentPage + 1}`,
    })
    return {
      title: `#${i + 1} · ${item.title}`.slice(0, 80),
      body: [item.source ? `Fuente: ${item.source}` : '', item.duration ? `Duración: ${item.duration}` : '', `Página ${currentPage}`].filter(Boolean).join(' · ').slice(0, 120),
...
async function download(ctx: CommandContext, value: string) {
  let title = 'Hentai'
  try { title = getHentaiItem(value).title } catch { /* direct URL */ }
  const status = await ctx.reply(`⬇️ *HENTAI · PREPARANDO*\n${title}`)
  try {
    ...
    if (key) await ctx.socket.sendMessage(ctx.chatId, { text: `✅ *HENTAI ENVIADO*\n${result.title}\n📦 ${(result.size / 1024 / 1024).toFixed(1)} MB` }, { edit: key as never }).catch(() => undefined)
    await result.cleanup()
  } catch {
    const key = (status as { key?: unknown }).key
    if (key) await ctx.socket.sendMessage(ctx.chatId, { text: `❌ *NO SE PUDO DESCARGAR*\n${title}\nLa descarga no pudo completarse. Inténtalo nuevamente.` }, { edit: key as never }).catch(() => undefined)
  }
}

export const hentaiV9Commands: BotCommand[] = [{
  name: 'hentai', aliases: ['h', 'hanime'], category: 'adult',
  description: 'Busca y descarga contenido hentai público con carrusel y navegación integrada.',
  ...
  async handler(ctx) {
    ...
    if (action === 'search') {
      ...
      await show(ctx, 'search', 1, query); return
    }
    ...
    if (action === 'dl' || action === 'download') {
      ...
      await download(ctx, value); return
    }
