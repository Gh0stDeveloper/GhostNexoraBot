...
function parseCoord(s: string) {
  const n = Number(s)
  if (!Number.isFinite(n)) throw new Error('Coordenada invalida: ' + s)
  return Math.trunc(n)
}

const FOOTER = 'Ghost Nexora Bot · Ghost Developer'
...
export const minecraftCommands: BotCommand[] = [
  {
    ...
    async handler(ctx) {
      await ctx.reply(
        [
          'Minecraft · Ghost Nexora',
          '--------------',
          '.mcseed <seed> — strongholds anillo 1',
          ...
          '.mcalert <item> <precio> · list · del <id>',
          '.mcprice <item> <precio> — dispara alertas',
          '',
          'Seeds/estructuras: estimacion determinista (no cubiomes 1:1).',
          'Skins/UUID/servidor: datos reales Mojang + SLP.',
          FOOTER,
        ].join('\n'),
      )
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      const lines = [
        ...
        '',
        'Estimacion por anillos oficiales + PRNG. Confirma con /locate.',
        FOOTER,
      ]
    ...
    async handler(ctx) {
      ...
      await ctx.reply(
        [
          ...
          '',
          'Lleva Ojos de Ender; portal room suele estar bajo Y=35.',
          FOOTER,
        ].join('\n'),
      )
    },
    ...
    async handler(ctx) {
      ...
      await ctx.reply(
        [
          ...
          r.tip,
          '',
          'Matematica exacta del juego (Overworld / 8 = Nether).',
          FOOTER,
        ].join('\n'),
      )
    },
    ...
    async handler(ctx) {
      ...
      await ctx.reply(
        [
          ...
          '',
          'Candidatos por grid+salt (estilo Java). Confirma con /locate.',
          FOOTER,
        ].join('\n'),
      )
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      await ctx.socket.sendMessage(ctx.chatId, {
        image: { url: urls.body },
        caption: [profile.name, 'UUID: ' + profile.uuid, 'Skin: ' + urls.skin, FOOTER].join('\n'),
      })
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      await ctx.socket.sendMessage(ctx.chatId, {
        ...
        caption: [
          ...
          skinInfo,
          '',
          'Historial de nombres: Mojang ya no lo publica en API publica.',
          FOOTER,
        ].join('\n'),
      })
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      const { host, port } = parseHostPort(
        ctx.args.join('').includes(':') ? ctx.args[0]! : ctx.argText.trim(),
      )
  ...
  },
  {
    ...
    category: 'tools',
    description: 'Como craftear un item.',
    usage: 'mccraft <item>',
    async handler(ctx) {
      ...
      if (!r.found) {
        await ctx.reply([item, r.tip, 'Wiki: ' + r.wiki, FOOTER].join('\n'))
        return
      }
      await ctx.reply(
        ['Crafteo', r.result, '', 'Patron:', ...r.shape, 'Wiki: ' + r.wiki, FOOTER].join('\n'),
      )
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      if (sub === 'del' || sub === 'rm' || sub === 'delete') {
        ...
        await ctx.reply(n ? 'Eliminada(s): ' + n : 'No encontre esa alerta.')
        return
      }
      ...
      await ctx.reply(
        [
          ...
          'ID: ' + row.id,
          '',
          'No hay API global de economia no premium.',
          'Usa .mcprice <item> <precio> para reportar y avisar.',
          FOOTER,
        ].join('\n'),
      )
    },
  },
  {
    ...
    async handler(ctx) {
      ...
      const price = Number(ctx.args[ctx.args.length - 1])
      if (!Number.isFinite(price)) throw new Error('Precio invalido')
      const item = ctx.args.slice(0, -1).join(' ')
      ...
      await ctx.reply(
        [
          ...
          hits.length
            ? hits.length + ' alerta(s) coinciden (<= su maximo).'
            : 'Nadie tenia alerta para ese precio.',
          FOOTER,
        ].join('\n'),
      )
