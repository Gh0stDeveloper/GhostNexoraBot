...
function numberToProfession(input: string) {
  if (!/^\d+$/.test(input)) return input
  const index = Number(input) - 1
  const row = professionRows()[index]
  if (!row) throw new Error(`Profesión inválida. Usa ${numberedList('.')}`)
  return row[0]
}
function numberedList(prefix: string) {
  return professionRows().map(([id, p], i) => `${String(i + 1).padStart(2, '0')}. ${p.emoji} *${p.label}* · ${id}`).join('\n')
}
async function job(ctx: CommandContext) {
  const input = ctx.argText.trim()
  if (!input) {
    const current = professionsV2.get(ctx.sender)
    await ctx.reply(`💼 *PROFESIONES NEXORA*\n━━━━━━━━━━━━━━\nActual: ${current.emoji} *${current.label}*\n\n${numberedList(ctx.prefix)}\n\nSelecciona: *${ctx.prefix}job <número|nombre>*\nAl seleccionar una profesión bloqueada verás únicamente sus requisitos, sin mostrar precios en esta lista.`)
    return
  }
  const selectedId = numberToProfession(input)
  const id = resolveCareerId(selectedId) ?? selectedId
  const status = careerLicenses.status(ctx.sender, id)
  if (!status.unlocked) {
    await ctx.reply([
      `🔒 *${status.item.emoji} ${status.item.label.toUpperCase()}*`,
      '━━━━━━━━━━━━━━',
      'Esta profesión todavía está bloqueada.',
      `📋 Requisitos: ${status.requirement}`,
      '',
      `Completa los requisitos y después usa *${ctx.prefix}job ${status.profession}*.`,
      `También existe la opción de comprar el título con *${ctx.prefix}joblicense ${status.profession}*.`,
    ].join('\n'))
    return
  }
  ...
  await ctx.reply([
    ...
    '',
    `Ya puedes usar *${ctx.prefix}work* o *${ctx.prefix}w* para trabajar.`,
  ].join('\n'))
}
async function work(ctx: CommandContext) {
