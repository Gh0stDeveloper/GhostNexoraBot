...
async function searchCommand(ctx: CommandContext, source: OfficialSource) {
  ...
  if (query.length < 2) throw new Error(`Uso: ${ctx.prefix}${source.toLowerCase()} <aplicación>`)
...
async function downloadCommand(ctx: CommandContext) {
  ...
  if (!item) throw new Error(`Selecciona primero una APK con ${ctx.prefix}uptodown, ${ctx.prefix}liteapks o ${ctx.prefix}happymod.`)
...
export const officialApkV7Commands: BotCommand[] = [
  ...
  { name: 'apkofficialdl', aliases: ['officialapkdl'], category: 'downloads', description: 'Descarga una APK seleccionada desde su página oficial.', usage: 'apkofficialdl <token>', handler: downloadCommand },
]
