...
function help(prefix: string) {
  return ['╭━━〔 🧠 *MINI-LLM LOCAL* 〕━━╮', `┃ ${prefix}llm status`, `┃ ${prefix}llm progress`, `┃ ${prefix}llm docs`, `┃ ${prefix}llm add`, `┃ ${prefix}llm train`, `┃ ${prefix}llm ask <pregunta>`, `┃ ${prefix}llm auto on|off`, '╰━━━━━━━━━━━━━━━━━━╯', '', 'Envía o responde a un PDF, DOCX o TXT con *llm add* para incorporarlo al corpus.'].join('\n')
}
...
export const miniLlmCommands: BotCommand[] = [{
  ...
  async handler(ctx) {
    ...
    if (sub === 'status' || sub === 'progress' || sub === 'avance') {
      ...
      await ctx.reply(['╭━━〔 🧠 *MINI-LLM · AVANCE* 〕━━╮', `┃ Estado » *${s.learning ? 'ENTRENANDO' : 'EN ESPERA'}*`, `┃ Documentos » *${s.totalDocuments}*`, `┃ Fragmentos » *${s.totalChunks}*`, `┃ Mensajes » *${s.totalMessages}*`, `┃ Pendientes » *${s.pendingMessages}*`, `┃ Vectores » *${s.vectorRecords}*`, `┃ Vocabulario » *${s.vocabSize}*`, `┃ Entrenamientos » *${s.trainRuns}*`, `┃ Pasos realizados » *${s.trainSteps}*`, `┃ Modelo » *v${s.modelVersion}*`, `┃ Último loss » *${s.lastLoss?.toFixed(5) ?? 'N/D'}*`, `┃ Auto-train » *${s.autoTrainEnabled ? 'ON' : 'OFF'}*`, `┃ Almacenamiento » *${formatBytes(s.storageBytes)}*`, '╰━━━━━━━━━━━━━━━━━━╯'].join('\n'))
      return
    }
    if (sub === 'docs' || sub === 'documentos') {
      const docs = miniLLM.listDocuments(); if (!docs.length) { await ctx.reply('No hay documentos cargados en el corpus local.'); return }
      const visible = docs.slice(-30).map((doc, i) => `${i + 1}. ${doc.name} · ${formatBytes(doc.size)}`)
      await ctx.reply(`📚 *CORPUS LOCAL*\n━━━━━━━━━━━━━━\n${visible.join('\n')}\n\nTotal visible: *${visible.length}*`); return
    }
    if (sub === 'add' || sub === 'agregar' || sub === 'document') {
      ...
      const media = await downloadMessageMedia(ctx.message); if (!media || media.kind !== 'document') throw new Error('Envía o responde a un PDF, DOCX o TXT con el comando.')
    ...
    if (sub === 'train' || sub === 'entrenar') {
      ...
      if (miniLLM.stats().learning) { await ctx.reply('🧠 Ya hay un entrenamiento en ejecución. Consulta *' + ctx.prefix + 'llm status*.'); return }
      await ctx.reply(`🧠 *ENTRENAMIENTO INICIADO*\n━━━━━━━━━━━━━━\nEl proceso continúa en segundo plano.\nConsulta el progreso con *${ctx.prefix}llm status*.`)
      void miniLLM.train('manual').catch(async (error) => { await ctx.reply('❌ El entrenamiento local falló. Revisa los logs del bot.').catch(() => undefined); console.error(error) })
      return
    }
    if (sub === 'ask' || sub === 'pregunta' || sub === 'query') {
      ...
      await ctx.reply(`🧠 Buscando en la memoria local: *${query}*...`); await ctx.reply(miniLLM.answer(query)); return
    }
    if (sub === 'auto') {
