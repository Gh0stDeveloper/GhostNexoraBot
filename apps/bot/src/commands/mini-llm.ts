...
function progressText() {
  ...
  const training = trainingQueueStatus()
  return [
    '╭━━〔 🧠 *LLM · PROGRESO* 〕━━╮',
...
export const miniLlmCommands: BotCommand[] = [{
  ...
  async handler(ctx) {
    ...
    if (sub === 'status' || sub === 'progress' || sub === 'avance') {
      const s = miniLLM.stats(); const q = getQueueState().jobs; const rq = trainingQueueStatus()
      const state = s.learning ? 'ENTRENANDO' : rq.requested ? 'EN COLA' : 'EN ESPERA'
      const pct = s.learning ? `${s.currentProgress}% · ${s.currentStep}/${Math.max(1, s.currentTotalSteps)}` : rq.requested ? 'EN COLA' : '100%'
      await ctx.reply([
        '╭━━〔 🧠 *MINI-LLM · ESTADO* 〕━━╮',
        `┃ Estado » *${state}*`, `┃ Progreso » *${pct}*`, `┃ Fase » *${s.currentMessage}*`,
        `┃ Documentos » *${s.totalDocuments}*`, `┃ Fragmentos » *${s.totalChunks}*`, `┃ Vectores » *${s.vectorRecords}*`,
        `┃ Vocabulario » *${s.vocabSize}/${miniLLM.constants.VOCAB_LIMIT}*`, `┃ Entrenamientos » *${s.trainRuns}*`,
        `┃ Pasos » *${s.trainSteps}*`, `┃ Último loss » *${s.lastLoss?.toFixed(5) ?? 'N/D'}*`,
        `┃ Cola docs » *${q.filter((j) => j.status === 'queued').length}*`, `┃ Almacenamiento » *${formatBytes(s.storageBytes)}*`,
        '╰━━━━━━━━━━━━━━━━━━╯',
      ].join('\n'))
      return
    }
    ...
    if (sub === 'search' || sub === 'buscar') {
      ...
      const hits = miniLLM.search(query, 5); if (!hits.length) { await ctx.reply('No se encontraron coincidencias.'); return }
