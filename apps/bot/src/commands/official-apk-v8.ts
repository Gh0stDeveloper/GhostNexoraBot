...
function token(source: Source, url: string) {
  return createHash('sha256').update(`${source}:${url}`).digest('hex').slice(0, 18)
}
...
async function getHtml(url: string) {
  const r = await fetch(url, {
    redirect: 'follow',
    headers: {
      'user-agent': 'GhostNexoraBot/0.0.7c',
...
function parse($: cheerio.CheerioAPI, source: Source, base: string, _query: string) {
  ...
  $('a[href]').each((_i, el) => {
    ...
    const icon = absolute(
      base,
      root.find('img').first().attr('data-src') || root.find('img').first().attr('src'),
    )
    const version = /\bv(?:ersion|ersión)?\s*[:.]?\s*([0-9][\w.+-]*)/i.exec(summary)?.[1]
    const item: Item = {
      token: token(source, href),
      source,
      name: text.replace(/\s+(APK|Mod APK|download)$/i, '').slice(0, 100),
      url: href,
      icon,
      version,
      summary,
...
async function searchSource(ctx: CommandContext, source: Source, query: string) {
  ...
  if (!items.length) throw new Error('No encontré resultados en la página oficial.')
  ...
  await sendCarousel(ctx.socket, ctx.chatId, ctx.message, {
    ...
    cards: items.map((item) => ({
      ...
      body: [
        ...
        item.summary || '',
        '✅ Obtenido desde la web oficial.',
      ]
        .filter(Boolean)
        .join('\n'),
      imageUrl: item.icon,
      buttons: [
        { type: 'reply' as const, text: '⬇️ Descargar', id: `${ctx.prefix}apkdl ${item.token}` },
...
async function apk(ctx: CommandContext) {
  ...
  if (!all.length) {
    throw new Error(
      `No encontré resultados en Uptodown/LiteAPKS. Prueba ${ctx.prefix}happymod <app> para mods.`,
    )
  }

  await sendCarousel(ctx.socket, ctx.chatId, ctx.message, {
    title: '📦 ANDROID · FUENTES OFICIALES',
    body: `Resultados para *${query}*\nPara mods usa ${ctx.prefix}happymod`,
    footer: 'Uptodown · LiteAPKS · HappyMod vía .happymod',
    cards: all.map((item) => ({
      title: `${item.source} · ${item.name}`,
      body: [item.version ? `🔄 ${item.version}` : '', item.summary || ''].filter(Boolean).join('\n'),
      imageUrl: item.icon,
      buttons: [
        { type: 'reply' as const, text: '⬇️ Descargar', id: `${ctx.prefix}apkdl ${item.token}` },
...
async function info(ctx: CommandContext) {
  ...
  if (!item) {
    throw new Error(
      `Primero usa ${ctx.prefix}apk, ${ctx.prefix}uptodown o ${ctx.prefix}liteapks. Para HappyMod: ${ctx.prefix}happymodinfo <token>.`,
    )
  }
  await ctx.reply(
    `📦 *${item.name}*\n━━━━━━━━━━━━━━\nFuente: *${item.source}*\n${item.version ? `Versión: ${item.version}\n` : ''}${item.summary || ''}\n\n${item.url}`,
...
async function download(ctx: CommandContext) {
  ...
  if (!item) {
    throw new Error(
      `Primero selecciona una APK desde ${ctx.prefix}apk / ${ctx.prefix}uptodown / ${ctx.prefix}liteapks. Para HappyMod usa ${ctx.prefix}happymoddl <token>.`,
    )
  }
  ...
  $('a[href]').each((_i, el) => {
    ...
    const href = official(absolute(page.finalUrl, $(el).attr('href')) || '', item.source)
    if (href && /\.apk(?:$|[?#])/i.test(href)) candidate = href
  })

  if (!candidate) {
    throw new Error('La página oficial no proporcionó un enlace APK directo recuperable.')
  }

  const response = await fetch(candidate, {
    redirect: 'follow',
    headers: { 'user-agent': 'GhostNexoraBot/0.0.7c' },
    signal: AbortSignal.timeout(60_000),
  })
  if (!response.ok) throw new Error('La página oficial no pudo entregar la APK.')

  const data = Buffer.from(await response.arrayBuffer())
  if (!data.length) throw new Error('La APK recibida está vacía.')

  const fileName = `${item.name.replace(/[^\w.-]+/g, '-').slice(0, 60)}.apk`
...
export const officialApkV8Commands: BotCommand[] = [
  {
    ...
    aliases: ['apks'],
    category: 'downloads',
    description: 'Busca APKs en Uptodown y LiteAPKS. Para HappyMod usa .happymod.',
