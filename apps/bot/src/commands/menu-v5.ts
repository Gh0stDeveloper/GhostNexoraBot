...
const sectionTitles: Record<SectionId, string> = {
  knowledge: '🧠 *IA · BÚSQUEDA · CONOCIMIENTO*',
  youtube: '🎵 *DESCARGAS · YOUTUBE Y AUDIO*',
  downloads: '📲 *DESCARGAS · REDES Y ARCHIVOS*',
  general: '🌐 *GENERAL*',
  profile: '👤 *PERFIL*',
  progress: '🏆 *PROGRESO · TEMPORADAS · MUNDO*',
  economy: '🪙 *ECONOMÍA Y FINANZAS*',
  rpg: '📖 *RPG · CRAFTING · MASCOTAS*',
  games: '🎮 *JUEGOS Y APUESTAS NXC*',
  collection: '🌸 *GACHA Y COLECCIÓN*',
  social: '💞 *SOCIAL · REACCIONES · REPUTACIÓN*',
  stickers: '🎨 *STICKERS Y HERRAMIENTAS*',
  groups: '👥 *ADMINISTRACIÓN DE GRUPOS*',
  automation: '📊 *COMUNIDAD · AUTOMATIZACIÓN*',
  subbots: '🤖 *SUBBOTS*',
  adult: '🔞 *DESCARGAS Y CONTENIDO 18+*',
  personalization: '🎛️ *PERSONALIZACIÓN*',
  support: '🎫 *SOPORTE*',
  staff: '🛡️ *STAFF DEL BOT*',
...
function sectionFor(command: BotCommand): SectionId {
  ...
  for (const id of ['knowledge','youtube','progress','rpg','automation','support','personalization'] as const) {
    if (sets[id].has(name)) return id
  }
...
function renderTokens(prefix: string, command: BotCommand, tokens: string[]) {
  const primary = `${prefix}${command.name}`
  ...
  const restriction = [command.groupOnly ? 'grupo' : '', command.adminOnly ? 'admin' : '', command.staffOnly ? 'staff' : '', command.ownerOnly ? 'owner' : '']
    .filter(Boolean).join('/')
...
export const menuV5Commands: BotCommand[] = [{
  name: 'menu', aliases: ['help','comandos'], category: 'general',
  description: 'Menú completo generado desde todos los comandos activos.', handler: menu,
}]
