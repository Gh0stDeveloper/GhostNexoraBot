...
if (process.env.NEXORA_INSTANCE_ROLE !== 'subbot') throw new Error('subbot-worker-termux.ts requiere NEXORA_INSTANCE_ROLE=subbot')
if (!config.isTermuxLite) throw new Error('subbot-worker-termux.ts requiere NEXORA_RUNTIME_PROFILE=termux-lite')
...
const allowedCommands = termuxLiteCommands.filter((command) =>
