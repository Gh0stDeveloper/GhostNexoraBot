...
function Ranking({ rows, locale, emptyLabel }: { rows: OpsUsageRank[]; locale: string; emptyLabel: string }) {
  if (!rows.length) return <div className="px-5 py-10 text-center text-sm text-zinc-600">{emptyLabel}</div>
...
export function OpsUsageDashboard({ analytics, instanceLabel, locale }: Props) {
  ...
  return <section className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.55fr_.85fr]">
      <article className="ops-panel overflow-hidden"><div className="flex items-center justify-between border-b border-white/[.07] px-5 py-4"><div><h3 className="font-bold text-zinc-100">{labels.chart}</h3><p className="mt-1 text-xs text-zinc-600">60 min · {labels.max} {chart.max.toLocaleString(numberLocale)}</p></div>{latestPoint && <span className="font-mono text-[10px] text-zinc-600">{timeFormatter.format(new Date(latestPoint.at))}</span>}</div><div className="p-4"><svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="h-64 w-full" role="img" aria-label={labels.chart}><line x1="28" y1="202" x2="972" y2="202" stroke="currentColor" className="text-white/[.08]"/><line x1="28" y1="110" x2="972" y2="110" stroke="currentColor" className="text-white/[.05]"/><line x1="28" y1="18" x2="972" y2="18" stroke="currentColor" className="text-white/[.05]"/><polyline points={chart.points} fill="none" stroke="currentColor" strokeWidth="4" strokeLinejoin="round" strokeLinecap="round" className="text-blue-400"/></svg></div></article>
