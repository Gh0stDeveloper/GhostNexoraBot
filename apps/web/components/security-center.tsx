...
export function SecurityCenter({ locale }: { locale: WebLocale }) {
  ...
  useEffect(() => {
    refresh().catch(() => setError(t('security.failed')))
  }, [refresh, t('security.failed')])
  ...
  async function registerPasskey() {
    ...
    setError('')
    try {
      const name = window.prompt(t('security.deviceName'), t('security.defaultDevice'))?.trim() || t('security.defaultDevice')
  ...
  if (!snapshot) {
    return <section className="ops-panel p-6 text-sm text-zinc-500">
      <LoaderCircle className="mr-2 inline size-4 animate-spin"/>{error || t('security.loading')}
    </section>
  }

  return <div className="space-y-6">
    {snapshot.mfaPending ? <div className="rounded-xl border border-amber-500/20 bg-amber-500/[.07] px-4 py-3 text-sm text-amber-200">{t('security.enrollmentRequired')}</div> : null}
    <section className="ops-panel overflow-hidden">
      <div className="border-b border-white/[.08] px-5 py-5">
        <div className="flex items-center gap-3"><ShieldCheck className="size-5 text-blue-400"/><div><h2 className="font-bold text-white">{t('security.title')}</h2><p className="mt-1 text-xs text-zinc-500">{t('security.subtitle')}</p></div></div>
      </div>
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-2">
        <article className="ops-node">
          <div className="flex items-center gap-2 font-bold text-white"><Fingerprint className="size-4 text-blue-400"/>{t('security.passkeys')}</div>
          <p className="mt-2 text-xs leading-5 text-zinc-500">{t('security.passkeyText')}</p>
          ...
          <div className="mt-4 space-y-2">
            ...
            </div>) : <p className="text-xs text-zinc-600">{t('security.noPasskeys')}</p>}
          </div>
        </article>

        <article className="ops-node">
          <div className="flex items-center gap-2 font-bold text-white"><KeyRound className="size-4 text-blue-400"/>{t('security.sessions')}</div>
    ...
    {snapshot.role === 'owner' ? <section className="ops-panel overflow-hidden">
      ...
      <div className="p-5">
        <div className="grid gap-2 md:grid-cols-[1fr_180px_auto]">
          <input value={label} onChange={(event) => setLabel(event.target.value)} className="ops-input" placeholder={t('security.label')}/>
          <select value={role} onChange={(event) => setRole(event.target.value as 'admin' | 'support')} className="ops-input">
            <option value="support">{t('security.roleSupport')}</option>
