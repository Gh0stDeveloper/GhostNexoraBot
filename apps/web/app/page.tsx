  ...
  return <main className="ops-page">
    <header className="sticky top-0 z-30 border-b border-white/[.07] bg-[#080809]/90 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-[1480px] items-center justify-between gap-4 px-5 py-4 md:px-8">
        ...
        <nav className="hidden gap-6 text-xs font-semibold text-zinc-500 md:flex"><a href="#arquitectura" className="hover:text-white">Arquitectura</a><a href="#modulos" className="hover:text-white">Módulos</a><a href="#seguridad" className="hover:text-white">Seguridad</a></nav>
    ...
    <section id="inicio" className="mx-auto grid min-h-[72vh] w-full max-w-[1480px] items-center gap-12 px-5 py-16 md:px-8 lg:grid-cols-[1.05fr_.95fr] lg:py-24">
      <div>
        ...
        <h1 className="mt-5 max-w-4xl text-5xl font-black leading-[.98] tracking-[-.05em] text-white sm:text-6xl lg:text-7xl">Un bot de WhatsApp con arquitectura de <span className="text-blue-400">plataforma.</span></h1>
        <p className="mt-6 max-w-2xl text-base leading-7 text-zinc-500">Ghost Nexora Bot integra administración de comunidades, economía, descargas, juegos, automatización, subbots y un stack opcional de IA. El panel operativo permite observar rendimiento y administrar cada instancia sin mezclar sus datos.</p>
        <div className="mt-8 flex flex-wrap gap-3"><a href="#arquitectura" className="ops-button-primary"><GitBranch className="size-4"/>Ver arquitectura</a><a href="/login?mode=subbot" className="ops-button-muted"><LayoutDashboard className="size-4"/>Portal de subbot</a></div>
    ...
    <section id="arquitectura" className="mx-auto w-full max-w-[1480px] scroll-mt-24 px-5 py-16 md:px-8">
      <div className="ops-panel overflow-hidden">
        <div className="border-b border-white/[.08] px-5 py-5"><p className="font-mono text-[10px] font-bold uppercase tracking-[.16em] text-blue-500">PIPELINE DAG</p><h2 className="mt-2 text-xl font-black">Arquitectura de procesamiento</h2><p className="mt-2 max-w-3xl text-sm text-zinc-600">La misma topología que el Operations Center audita en producción. Los visitantes ven la arquitectura; las métricas reales permanecen dentro de los paneles autenticados.</p></div>
